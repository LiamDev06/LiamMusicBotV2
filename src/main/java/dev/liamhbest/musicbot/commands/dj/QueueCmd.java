...
public class QueueCmd extends MusicCommand
{
    ...
    public void doCommand(CommandEvent event)
    {
        ...
        builder.setText((i1,i2) -> getQueueTitle(ah, event.getClient().getSuccess(), songs.length, fintotal, settings.getRepeatMode()))
                ...
                .setColor(new Color(28, 124, 60));

        builder.build().paginate(event.getChannel(), pagenum);
    }
    
    private String getQueueTitle(AudioHandler ah, String success, int songslength, long total, LoopMode repeatmode) {
        StringBuilder sb = new StringBuilder();

        if (ah.getPlayer().getPlayingTrack() != null) {
            sb.append(ah.getPlayer().isPaused() ? LiamMusicBot.PAUSE_EMOJI : LiamMusicBot.PLAY_EMOJI).append(" **")
