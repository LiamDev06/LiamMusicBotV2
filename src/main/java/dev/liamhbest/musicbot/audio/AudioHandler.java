...
import dev.liamhbest.musicbot.queue.FairQueue;
...
public class AudioHandler extends AudioEventAdapter implements AudioSendHandler {
    ...
    public void onTrackEnd(AudioPlayer player, AudioTrack track, AudioTrackEndReason endReason) {
        LoopMode loopMode = manager.getBot().getSettingsManager().getSettings(guildId).getRepeatMode();
        // if the track ended normally, and we're in repeat mode, re-add it to the queue
        if (endReason == AudioTrackEndReason.FINISHED && loopMode == LoopMode.ENABLED) {
    ...
    public void onTrackStart(AudioPlayer player, AudioTrack track) {
        ...
        if (settings.getRepeatMode() == LoopMode.ENABLED) {
            embed.setColor(Color.YELLOW);
            embed.setTitle(":infinity: Looping Song...");
        } else {
        ...
        embed.appendDescription(title + " - " + author);

        if (settings.getNowPlayingChannel(manager.getBot().getJDA().getGuildById(guildId)) != null) {
            TextChannel channel = settings.getNowPlayingChannel(manager.getBot().getJDA().getGuildById(guildId));
            channel.sendMessageEmbeds(embed.build()).queue();
        } else {
            manager.getNowPlayingChannelCache(guildId).sendMessageEmbeds(embed.build()).queue();
        }
    }
    ...
    public Message getNowPlaying(JDA jda) {
        if (isMusicPlaying(jda)) {
            ...
            eb.setColor(new Color(28, 124, 60));
            ...
            eb.setDescription((audioPlayer.isPaused() ? LiamMusicBot.PAUSE_EMOJI : LiamMusicBot.PLAY_EMOJI)
    ...
    public Message getNoMusicPlaying(JDA jda) {
        ...
        return new MessageBuilder()
                ...
                .setEmbed(new EmbedBuilder()
                        ...
                        .setDescription(LiamMusicBot.STOP_EMOJI + " " + FormatUtil.progressBar(- 1) + " " + FormatUtil.volumeIcon(audioPlayer.getVolume()))
    ...
    public String getTopicFormat(JDA jda) {
        if (isMusicPlaying(jda)) {
            ...
            return "**" + title + "** [" + (userid == 0 ? "autoplay" : "<@" + userid + ">") + "]"
                    + "\n" + (audioPlayer.isPaused() ? LiamMusicBot.PAUSE_EMOJI : LiamMusicBot.PLAY_EMOJI) + " "
                    + "[" + FormatUtil.formatTime(track.getDuration()) + "] "
                    + FormatUtil.volumeIcon(audioPlayer.getVolume());
        } else {
            return "No music playing " + LiamMusicBot.STOP_EMOJI + " " + FormatUtil.volumeIcon(audioPlayer.getVolume());
        }
    }
